...
const { logArray, displayHelp, displayTips, cookieMonster } = require("./src/utils");
...
function startServer(isSecure) {
  ...
  function handleViewers(req, response) {
    ...
    req.on("close", function () {
      ...
      console.log(
        `Viewer ${connectionId} left. Total clients: (${Object.keys(viewers).length})`
      );
    });
  }
}
...
function storeLog(filename, request) {
  ...
  fs.writeFile(`./logs/${filename}.json`, JSON.stringify(request, null, 2), () => {});
}
...
function setHeaders(req, res, next) {
  ...
  res.setHeader("X-hacker", "This is a tool for testing requests, please don't hack.");
